...
fn nearest_neighbor_interpolate(image: ArrayView2<f64>, scale_factor: f64) -> Array2<f64> {
...
fn bilinear_interpolate(image: ArrayView2<f64>, scale_factor: f64) -> Array2<f64> {
    let (height, width) = image.dim();
    let new_height = (height as f64 * scale_factor) as usize;
    let new_width = (width as f64 * scale_factor) as usize;
    let mut new_image = Array2::zeros((new_height, new_width));
...
#[pyfunction]
fn py_nearest_neighbor_interpolate<'py>(
    py: Python<'py>,
    image: PyReadonlyArray2<'py, f64>,
    scale_factor: f64,
) -> Bound<'py, PyArray2<f64>> {
    let image_array = image.as_array();
    let interpolated_array = nearest_neighbor_interpolate(image_array, scale_factor);
    interpolated_array.to_pyarray(py)
}

#[pyfunction]
fn py_bilinear_interpolate<'py>(
    py: Python<'py>,
    image: PyReadonlyArray2<'py, f64>,
    scale_factor: f64,
) -> Bound<'py, PyArray2<f64>> {
    let image_array = image.as_array();
    let interpolated_array = bilinear_interpolate(image_array, scale_factor);
    interpolated_array.to_pyarray(py)
}

#[pymodule]
fn image_interpolation(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(py_nearest_neighbor_interpolate, m)?)?;
    m.add_function(wrap_pyfunction!(py_bilinear_interpolate, m)?)?;
    Ok(())
}
